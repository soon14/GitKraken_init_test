 ...
 * 
 */
public class ParkingEntranceRecord1  {
	private Integer recordId=0;  //记录ID
	private Integer oldRecordId=0; //记录ID(外部数据库)
	private Integer carParkId=0;	//停车场ID
	private String licensePlateNumber; //车牌号
	private String imgName; //车牌图片名称
	private String inTime;  //进入停车记录时间
	private String inTimeStr; //进入停车场记录时间字符串;
	private Integer discountRate = 0; //折扣率, 以百分比为单位的折扣率，例如折扣为80%，则该值为80
	private Integer discountTime = 0; //折扣时间 ,单位为分
	private Integer discountMoney = 0; //折扣款 ,单位为人民币分
	private java.util.Date addTime;//   添加时间
	private String addTimeStr; //添加时间字符串
	private Integer mark =0; //出场标记  默认 0:没有出场，1:已出场
	private Integer IsRecognition =0;//是否识别 默认0：未识别，1：已识别
	private String recognitionNumber;//识别后的车牌号
	private Integer groupId=0;	//集团Id
	private Integer mallId=0;	//商场Id
