...
@Service("savechaegeToCacheService")
public class SaveChargeToCacheServiceImp implements SaveCarChargeCacheService {

	
	private static ResourceBundle res = ResourceBundle.getBundle("config");
	
	final static Object object = new Object();
	private final static Logger logger = Logger.getLogger(SaveChargeToCacheServiceImp.class);
	...
	public void startsave() {
		System.out.println("savechaegeToCacheService init start");
		while (true) {
			try {

				// logger.info("===============1=====================");
				List<String> list = JedisPoolUtils.lrange(IPutil.getIPaddress() + "_" + DataConstants.CARCHARGEDATA, 0, 50);
				// logger.info("===============2=====================");
				if (list.size() > 0) {
					...
					IBuildCallback callback = new IBuildCallback() {
						...
						public void savecarin(String rmdata, int state) {
							...
							if (state > -1) {
								...
								JedisPoolUtils.lrem(IPutil.getIPaddress() + "_" + DataConstants.CARCHARGEDATA, 1, rmdata);
								// taskMapper.updateByparkingNo(carparkNo);
							} else {
								logger.info("失败，删除插入redis另一张表里面===========" + rmdata);
								JedisPoolUtils.lrem(IPutil.getIPaddress() + "_" + DataConstants.CARCHARGEDATA, 1, rmdata);
								JedisPoolUtils.rpush(IPutil.getIPaddress() + "_" + DataConstants.CARCHARGEDATAERROR, rmdata);
							}
	...
	public Object saveData(String requestbody, IBuildCallback callback) {
		...
		// boolean flag=false;
		try {
			result = parkOutInService.uploadBoxPayRecord(requestbody);
	...
	@Override
	public String saveCarChargeredis(String requestBody) {
		...
		try {
			...
			JedisPoolUtils.rpush(ip + "_" + DataConstants.CARCHARGEDATA, requestBody);
