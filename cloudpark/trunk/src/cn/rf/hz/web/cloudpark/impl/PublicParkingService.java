...
public class PublicParkingService {
	...
	public Tc_userinfo getuserinfo(String licensePlateNumber, String ParkingLotNo, Date datetime, String channelId) {
		...
		try {
			...
			for (int i = 0; i < 3; i++) {
				resultjson = sendpost(posturl, CarProp.toJSONString());
				...
				if (resultjson == null || resultjson.isEmpty()) {
					...
					if (i > 0) {
						log.info("PublicParkingService.sendpost.exception." + licensePlateNumber + ":" + resultjson
								+ " ,count:" + i);
					}
					break;
				}
			...
			if (result_.getString("isLongCar").equals("1")) {
				...
				/*
				 * log.info("objectobjectobjectobject" + object.toJSONString());
				 * log.info("bgndt" + object.getString("bgndt"));
				 * log.info("enddt" + object.getString("enddt"));
				 * log.info("ruleId" + object.getString("ruleId"));
				 * log.info("username" +object.getString("username"));
				 * log.info("userpropertiy" +object.getString("userpropertiy"));
				 */
		...
		// log.info("getBgndtgetBgndtgetBgndtgetBgndt" + user.getBgndt());
		return user;
	}

	public String sendpost(String url, String param) {
		log.info("urlurlurlurl" + url);
	...
	public ArrayList<Tc_usercrdtm> setUsercrdtm(String requestBody) {
		log.info("PublicParkingService.setUsercrdtm" + requestBody);
		...
		for (int i = 0; i < dataArrays.size(); i++) {
			...
			log.info(usercrdtm.getCarcode() + "PublicParkingService.Carcode" + usercrdtm.getCarcode());
			log.info(usercrdtm.getCarcode() + "PublicParkingService.getParkinglotno" + usercrdtm.getParkinglotno());
			 ...
			 */
			log.info(usercrdtm.getCarcode() + "PublicParkingService.Username:" + usercrdtm.getUsername());
			usercrdtm.setChargeruleid(getUserRuleId(userinf, usercrdtm.getChannelid()));
			log.info(usercrdtm.getCarcode() + "PublicParkingService.Chargeruleid:" + usercrdtm.getChargeruleid());
			...
			if (usercrdtm.getCrdtm().getTime() == lastOutTime.getTime()) {
				log.info(usercrdtm.getCarcode() + "PublicParkingService.setUsercrdtm.lastOutTime == crdtm,userinfo:"
						+ userinf);
				if (userinf != null) {
