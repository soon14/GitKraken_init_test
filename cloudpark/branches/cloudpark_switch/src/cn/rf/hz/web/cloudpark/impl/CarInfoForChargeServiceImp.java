...
public class CarInfoForChargeServiceImp implements CarInfoForChargeService {
	...
	@Autowired
	private CarInfoForPadService carInfoForPadService;
	...
	private JSONObject getCarInfo(String parkNo, String carCode, String inTimeStr, int channelId, int carInType)
		...
		if (binaryCarInType == 1) {
			...
			// LOG.info("CarInfoForChargeService.getCarInfo从redis中取长期用户信息:" +
			// longUserStr);
			// 获取长期车数据
		...
		// 判断是不是为场中场或者场内切换
		ParkingLotParameter pp = carInfoForPadService.getParkingParamInfo(parkNo);
		...
		if (isInparkingOrSwitch) {
			...
			LOG.info("CarInfoForChargeService.array_crosspoint:" + carCode + "_" + array_crosspoint);
