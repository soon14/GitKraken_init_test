...
public class PadInfoServiceImp implements PadInfoService {
	...
	public JSONObject getPadInfo(String requestBody) {
		...
		try {
			...
			if (station != null && station.getRecordid() > 0) {
				...
				LOG.info("getPadInfo,isReductionSuperposition:" + parkingparm.getParmvalue());
	...
	public JSONObject getNewPadInfo(String requestBody) {
		...
		try {
			...
			Pb_station station = stationMapper.selectBySerialNumber(serialnumber);
			if (station != null && station.getRecordid() > 0) {
				String parktype = parkinginfoMapper.queryParkingTypeById(station.getParkinglotno());
				String parkinglotno = "";
				Pb_parkingbox parkingbox = parkingboxMapper.selectByPrimaryKey(station.getChannelid());
				if ("0".equals(parktype)) {
					parkinglotno = station.getParkinglotno();
				...
				model.put("parkNo", parkinglotno);
				model.put("areaId", parkingbox.getParkingareaid());
